...
function PropsSI(FluidName::AbstractString, Output::AbstractString)
  val = ccall( (:Props1SI, libcoolprop), Cdouble, (Ptr{UInt8},Ptr{UInt8}), FluidName,Output)
  if val == Inf
...
function PropsSI(Output::AbstractString, Name1::AbstractString, Value1::Real, Name2::AbstractString, Value2::Real, Fluid::AbstractString)
  val = ccall( (:PropsSI, libcoolprop), Cdouble, (Ptr{UInt8},Ptr{UInt8},Float64,Ptr{UInt8},Float64,Ptr{UInt8}), Output,Name1,Value1,Name2,Value2,Fluid)
  if val == Inf
    error("CoolProp: ", get_global_param_string("errstring"))
  end
  return val
...
function PhaseSI(Name1::AbstractString, Value1::Real, Name2::AbstractString, Value2::Real, Fluid::AbstractString)
  val = ccall( (:PhaseSI, libcoolprop), Int32, (Ptr{UInt8},Float64,Ptr{UInt8},Float64,Ptr{UInt8}, Ptr{UInt8}, Int), Name1,Value1,Name2,Value2,Fluid,message_buffer::Array{UInt8,1},buffer_length)
...
function get_global_param_string(Key::AbstractString)
  val = ccall( (:get_global_param_string, libcoolprop), Clong, (Ptr{UInt8},Ptr{UInt8},Int), Key,message_buffer::Array{UInt8,1},buffer_length)
  if val == 0
    error("CoolProp: ", get_global_param_string("errstring"))
  end
...
#   message_buffer[1:length(OutType)+1] = [OutType.data; 0x00]
  message_buffer[1:length(OutType)+1] = [codeunits(OutType); 0x00]
  val = ccall( (:get_parameter_information_string, libcoolprop), Clong, (Ptr{UInt8},Ptr{UInt8},Int), Key,message_buffer::Array{UInt8,1},buffer_length)
  if val == 0
    error("CoolProp: ", get_global_param_string("errstring"))
  end
  return unsafe_string(convert(Ptr{UInt8}, pointer(message_buffer::Array{UInt8,1})))
...
#  val = ccall( (:get_mixture_binary_pair_data, libcoolprop), Clong, (Ptr{UInt8},Ptr{UInt8},Ptr{UInt8}), CAS1,CAS2,Key)
#  return val
#end

# CoolProp::get_fluid_param_string
function get_fluid_param_string(fluid::AbstractString,param::AbstractString)
  val = ccall( (:get_fluid_param_string, libcoolprop), Clong, (Ptr{UInt8},Ptr{UInt8},Ptr{UInt8},Int), fluid,param,message_buffer::Array{UInt8,1},buffer_length)
  if val == 0
    error("CoolProp: ", get_global_param_string("errstring"))
  end
  return unsafe_string(convert(Ptr{UInt8}, pointer(message_buffer::Array{UInt8,1})))
...
function set_reference_stateS(Ref::AbstractString, reference_state::AbstractString)
  val = ccall( (:set_reference_stateS, libcoolprop), Cint, (Ptr{UInt8},Ptr{UInt8}), Ref,reference_state)
  if val == 0
    error("CoolProp: ", get_global_param_string("errstring"))
  end
  return val
...
function get_param_index(param::AbstractString)
  val = ccall( (:get_param_index, libcoolprop), Clong, (Ptr{UInt8},), param)
  if val == -1
...
function get_input_pair_index(param::AbstractString)
  val = ccall( (:get_input_pair_index, libcoolprop), Clong, (Ptr{UInt8},), param)
  if val == -1
...
function set_config_string(key::AbstractString,val::AbstractString)
  ccall( (:set_config_string, libcoolprop), Nothing, (Ptr{UInt8},Ptr{UInt8}), key,val)
...
function HAPropsSI(Output::AbstractString, Name1::AbstractString, Value1::Real, Name2::AbstractString, Value2::Real, Name3::AbstractString, Value3::Real)
  val = ccall( (:HAPropsSI, libcoolprop), Cdouble, (Ptr{UInt8},Ptr{UInt8},Float64,Ptr{UInt8},Float64,Ptr{UInt8},Float64), Output,Name1,Value1,Name2,Value2,Name3,Value3)
  if val == Inf
    error("CoolProp: ", get_global_param_string("errstring"))
  end
  return val
...
function AbstractState_factory(backend::AbstractString, fluids::AbstractString)
  AbstractState = ccall( (:AbstractState_factory, libcoolprop), Clong, (Ptr{UInt8},Ptr{UInt8},Ref{Clong},Ptr{UInt8},Clong), backend,fluids,errcode,message_buffer::Array{UInt8,1},buffer_length)
  if errcode[] != 0
    if errcode[] == 1
      error("CoolProp: ", string(convert(Ptr{UInt8}, pointer(message_buffer))))
    elseif errcode[] == 2
...
function AbstractState_specify_phase(handle::Clong,phase::AbstractString)
  ccall( (:AbstractState_specify_phase, libcoolprop), Nothing, (Clong,Ptr{UInt8},Ref{Clong},Ptr{UInt8},Clong), handle,phase,errcode,message_buffer::Array{UInt8,1},buffer_length)
  if errcode[] != 0
    if errcode[] == 1
      error("CoolProp: ", unsafe_string(convert(Ptr{UInt8}, pointer(message_buffer))))
    elseif errcode[] == 2
...
function  AbstractState_update_and_common_out(handle::Clong, input_pair::Clong, value1::Array{R1}, value2::Array{R2}, length::Real, T::Array{F}, p::Array{F}, rhomolar::Array{F}, hmolar::Array{F}, smolar::Array{F}) where {R1 <: Real, R2 <: Real, F <: AbstractFloat}  
...
function AbstractState_update_and_common_out(handle::Clong, input_pair::AbstractString, value1::Array{R1}, value2::Array{R2}, length::Real, T::Array{F}, p::Array{F}, rhomolar::Array{F}, hmolar::Array{F}, smolar::Array{F}) where {R1 <: Real, R2 <: Real, F <: AbstractFloat}
...
function AbstractState_update_and_1_out(handle::Clong, input_pair::Clong, value1::Array{R1}, value2::Array{R2}, length::Real, output::Clong, out::Array{F}) where {R1 <: Real, R2 <: Real, F <: AbstractFloat}
...
function AbstractState_update_and_1_out(handle::Clong, input_pair::AbstractString, value1::Array{R1}, value2::Array{R2}, length::Real, output::AbstractString, out::Array{F}) where {R1 <: Real, R2 <: Real, F <: AbstractFloat}
...
function AbstractState_update_and_5_out(handle::Clong, input_pair::Clong, value1::Array{R1}, value2::Array{R2}, length::Real, outputs::Array{Clong}, out1::Array{F}, out2::Array{F}, out3::Array{F}, out4::Array{F}, out5::Array{F}) where {R1 <: Real, R2 <: Real, F <: AbstractFloat}
...
function AbstractState_update_and_5_out(handle::Clong, input_pair::AbstractString, value1::Array{R1}, value2::Array{R2}, length::Real, outputs::Array{S}, out1::Array{F}, out2::Array{F}, out3::Array{F}, out4::Array{F}, out5::Array{F}) where {R1 <: Real, R2 <: Real, S <: AbstractString, F <: AbstractFloat}
#   outputs_key = Array(Clong,5)
  outputs_key = Array{Clong,1}(UndefInitializer(),5)
  for k = 1:5
...
function AbstractState_set_binary_interaction_double(handle::Clong,i::Int, j::Int, parameter::AbstractString, value::Cdouble)
  ccall( (:AbstractState_set_binary_interaction_double, libcoolprop), Nothing, (Clong,Clong,Clong,Ptr{UInt8},Cdouble,Ref{Clong},Ptr{UInt8},Clong), handle,i,j,parameter,value,errcode,message_buffer::Array{UInt8,1},buffer_length)
  if errcode[] != 0
    if errcode[] == 1
      error("CoolProp: ", unsafe_string(convert(Ptr{UInt8}, pointer(message_buffer))))
    elseif errcode[] == 2
...
function AbstractState_set_fluid_parameter_double(handle::Clong, i::Int, parameter::AbstractString, value::Cdouble)
  ccall( (:AbstractState_set_fluid_parameter_double, libcoolprop), Nothing, (Clong,Clong,Ptr{UInt8},Cdouble,Ref{Clong},Ptr{UInt8},Clong), handle,i,parameter,value,errcode,message_buffer::Array{UInt8,1},buffer_length)
  if errcode[] != 0
    if errcode[] == 1
      error("CoolProp: ", unsafe_string(convert(Ptr{UInt8}, pointer(message_buffer))))
    elseif errcode[] == 2
